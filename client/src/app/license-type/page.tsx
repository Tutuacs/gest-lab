...
export default function LicenseTypeList() {
  ...
  const fetchData = async () => {
    const result = await fetchWithAuth(`/license-type/by-equipament/${equipamentTypeId}`);
  ...
  return (
    <main className="p-10 bg-gray-100 min-h-screen">
      <h1 className="text-4xl font-bold mb-8 text-center">Tipos de Licença</h1>
          ...
          <tbody>
            {licenses.map((license) => (
              <tr key={license.id} className="border-t hover:bg-gray-50">
                ...
                <td className="py-3 px-4">{new Date(license.createdAt).toLocaleDateString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
